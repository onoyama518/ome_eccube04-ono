<?php
/***********************************
  * リンクカウンタ         by ToR
  *  PHP4.1.0以降用
  * http://php.s3.to/
  * 2003/01/10
  **********************************
  * リンクカウンタです
  * ファイルに
  * ID|カウント|URL|登録日|タイトル|を記録します
  * 管理モードはJavaScriptを使用しています。
  * 指定リンク元以外はDLしないように出来ます
  * http://www.proxy2.deのtop_dlを参考にしました
  * 1/30 ポ→ﾎﾟ
  *link.php?id=[ID]でジャンプします。
  */
define("LOGFILE", "./linklog.txt");		/* ログファイル名(666) */

define("ADMINPASS", "ome_130307");		/* 管理パス */
define("SHOW_DEF",15);			/* ランクは最初何件表示？ */

define("BAR", "red.gif");		/* バーの画像 */
define("HEIGHT", 10);			/* 画像の高さ */

define("REFCHECK", 0);			/* リンク元チェックする？する場合↓ */
$okurl = array('http://www.ome-shouzai.com/ec/');	/* このリスト以外からのリンクは不可 */

define("TARGET",'target="_blank"');     /* ランキング時のリンク方法 */

/* へっだ部分 */
function head(){
  $this_day = gmdate("Y/m/d(D) H:i:s", time()+9*3600);//現在時刻

  echo <<<HEAD
<html>
<head>
<title>Link Counters</title>
<style type=text/css>
td { font-family: Arial, Helvetica, sans-serif; font-size: 8pt}
a:hover { color: #ff0000; text-decoration: underline }
select {  font-family: Arial, Helvetica, sans-serif; font-size: 8pt}
input {  font-size: 8pt}
tt {  font-family: Verdana, Arial; font-size: 13pt; font-weight: bold;color:f02080;}
a {  text-decoration: none}
</style>
<script language="Javascript">
<!--
  function ViewDetail(target,selObj) {
    eval(target+".location='{$_SERVER['PHP_SELF']}?job=detail&detail="+selObj.options[selObj.selectedIndex].value+"'");
  }
// -->
</script>
</head>
<body bgcolor="FFEEFF" text="#000000" link="#000066" vlink="#000066" alink="#000066">
<br>
<form method="post" action="{$_SERVER['PHP_SELF']}">
<table width="560" border="0" cellspacing="0" cellpadding="2" height="55" align="center">
  <tr> 
    <td> <tt>Link Counter</tt></td>
    <td align="right"> <img src="point.gif" width="9" height="9"><a href="{$_SERVER['PHP_SELF']}?job=admin">Administration</a></td>
  </tr>
  <tr>
    <td><img src="point.gif" width="9" height="9"><b>Date: {$this_day}</b></td>
    <td align="right">
       <select name="files" onChange="ViewDetail('self',this)">
         <option selected>show details</option>
HEAD;
}
/* フッタ部分 */
function foot() {
  echo <<<FOOT
 <table border=0 width="560" align="center" cellspacing="1" cellpadding="3">
  <tr> 
    <td valign="top">Link Counter by http://php.s3.to/</td>
      <td align="right">
       <select name="option">
         <option value="total" selected>Total</option>
         <option value="perDay">Per day</option>
       </select>
       <select name="top">
         <option value="10" selected>Top 10</option>
         <option value="15">Top 15</option>
         <option value="20">Top 20</option>
         <option value="30">Top 30</option>
         <option value="all">all</option>
       </select>
       <input type="submit" value="Submit">
    </td>
  </tr>
</table>
</form>
</body>
</html>
FOOT;
  exit;
}
/* カウントアップとDL */
function count_dl($id){

  $lines = file(LOGFILE);	//ファイル読み込み
  $find = FALSE;		//フラグ
  for($i=0; $i<count($lines); $i++){
    list($file_id,$count,$location,$stamp,$descript) = explode("|",$lines[$i]);
    if($id == $file_id){	//見つかったらカウントアップ
      $find=TRUE;
      $count++;
      $lines[$i] = "$file_id|$count|$location|$stamp|$descript|\n";//書き換え
      $redirect = $location;	//ファイル位置決定
      break;
    }
  }
  if($find){//ターゲットが見つかったらDL
    //ログ更新
    $fp = fopen(LOGFILE, "w");
    flock($fp, LOCK_EX);
    fputs($fp, implode("", $lines));
    fclose($fp);
    header("Location: $redirect");
  }else{
    die("対象ファイルが見つかりません！");
  }
  exit;
}
/* ランキング表示 */
function show_table($top,$option){

  head();//ヘッダ表示
  //ページ処理
  $lines = file(LOGFILE);
  $now = time();

  if($top == all) $top = count($lines);
  else $top = (preg_match("/^(\d+)/",$top)) ? $top : SHOW_DEF;
  //ソート用に読み込む
  for($i=0; $i<count($lines); $i++){
    list($file_id,$num,$location,$stamp,$descript) = explode("|",$lines[$i]);
    $descript = str_replace("ﾎﾟ", "ポ", $descript);
    if($option=="perDay"){
      $dl_perday = ($now-$stamp>86400) ? sprintf("%.2f",($num*86400/($now-$stamp))) : 0;
      $num_arr[$descript] = $dl_perday;	//ランク用
      $name_arr[$file_id] = $descript;	//右上選択用
      $ref_arr[$descript] = $location;	//参照用
      $total += $dl_perday;			//合計カウント計算
    }else{
      $num_arr[$descript] = $num;	//ランク用
      $name_arr[$file_id] = $descript;	//右上選択用
      $ref_arr[$descript] = $location;	//参照用
      $total += $num;			//合計カウント計算
    }
  }
  arsort($num_arr);			//数字の大きい順
  asort($name_arr);			//名前の小さい順
  //右上部分作成
  while(list($fileid,$desc)=each($name_arr)){
    echo "        <option value=\"$fileid\">$desc</option>\n";
  }
  //テーブルヘッダ
  echo "      </select>\n    </td>\n  </tr>\n</table>\n";
  echo "<table bgcolor=\"FFFFFD\" border=0 cellpadding=3 cellspacing=2 width=560 align=center>\n"; 
  echo "  <tr><td colspan=4 bgcolor=\"F1A8C1\"><font color=\"#990000\"><b>Top $top Linkers</b></font></td></tr>\n";
  //合計があったら
  if($total > 0){
    $flag = FALSE;	//一位フラグ
    while(list($title,$value)=each($num_arr)){
      if ($show < $top) {//上位何件
        $tmp = $rank;						//以前の数
        $rank = $value;						//新しい数
        if($tmp != $rank) $num = $show + 1;			//違うならランクアプ
        if(!$flag) $top_pos = $value;				//一位データ
        $per = sprintf("%.1f %%",($value *100 / $total));	//％フォーマット
        $img_width = ($value * 310 / $top_pos);			//バー幅決定
        if($ref_arr[$title]) $title = "<a href=\"$ref_arr[$title]\"".TARGET.">$title</a>";
        echo "<tr><td align=center><b>$num</b></td><td>$title</td><td nowrap><img src=\"".BAR."\" width=$img_width height=".HEIGHT."> $value </td><td>$per</td></tr>\n";
        $show++;						//表示カウント
        $flag = TRUE;
      }else{
        break;
      }
    }
  }
  foot();//フッタ表示
}
/* 詳細データ表示 */
function show_detail($id){

  head();//ヘッダ表示
  $now = time();
  //右上部分
  $lines = file(LOGFILE);
  for($i=0; $i<count($lines); $i++){
    list($file_id,$num,$location,$stamp,$descript,) = explode("|",$lines[$i]);
    $descript = str_replace("ﾎﾟ", "ポ", $descript);
    $name_arr[$file_id] = $descript;
    $total += $num;
  }
  asort($name_arr);
  while(list($fileid,$desc)=each($name_arr)){
    echo "        <option value=\"$fileid\">$desc</option>\n";
  }
  //テーブルヘッダ
  echo "      </select>\n    </td>\n  </tr>\n</table>\n";
  echo "<table bgcolor=\"F7F7F7\" border=0 cellpadding=3 cellspacing=2 width=560 align=center>\n"; 
  echo "  <tr><td colspan=4 bgcolor=\"F1A8C1\"><font color=\"990000\"><b>Top $top Linkers</b></font></td></tr>\n";
  //項目名の配列
  $details = array('ID','タイトル','登録日','経過日数','一日あたりのLink数','合計Link数','全体の割合','Link先');
  reset($lines);//一応リセット
  for($i=0; $i<count($lines); $i++){
    list($file_id,$count,$location,$stamp,$descript,) = explode("|",$lines[$i]);
    $descript = str_replace("ﾎﾟ", "ポ", $descript);
    if($id == $file_id){
      $file_date = date("Y/m/d(D) H:i:s", $stamp);	//データ登録日
      $total_days = (int)(($now-$stamp)/86400);		//経過日数
      //一日あたりのDL数
      $dl_perday = ($now-$stamp>86400) ? sprintf("%.2f",($count*86400/($now-$stamp))) : 0;
      $percent = sprintf("%.2f%%",($count*100/$total));//全体における割合
      $link = "<a href=\"$location\">$location</a>";
      break;
    }
  }
  //データの配列
  $vals = array($file_id,$descript,$file_date,$total_days,$dl_perday,$count,$percent,$link);
  //対応させて表示
  for($c=0; $c<count($details); $c++){
    echo "  <tr>\n    <td width=35% bgcolor=ffced5>$details[$c]</td>\n    <td bgcolor=ffe0e8>$vals[$c]</td>\n  </tr>\n";
  }
  
  echo "<tr><td colspan=2 align=left bgcolor=F1A8C1><a href=\"{$_SERVER['PHP_SELF']}\">&lt;&lt;Back</a></td></tr>";
  echo "</table>\n";
  foot();//フッタ表示
}
/* 管理画面表示　*/
function show_track(){
 
  $jump = $_SERVER['PHP_SELF']."?job=admin&admin=".md5(ADMINPASS);

  ?>
<html><head>
<meta http-equiv="Expires" content="Tue, 01 Jan 1980 1:00:00 GMT">
<meta http-equiv="Pragma" content="no-cache">
<title>Administration Mode</title>
<style type="text/css">
<!--
td {  font-family: Arial, Helvetica, sans-serif; font-size: 8pt}
a {  text-decoration: none; color: #000000}
a:hover { color: #000099; text-decoration: underline }
.input {  font-size: 8pt}
tt {  font-family: Verdana, Arial, Helvetica, sans-serif; font-size: 13pt; font-weight: bold}
//-->
</style>
<script language="Javascript">
<!--
function check_char(strg) {
 var found = 0;
 for (char_pos=0; char_pos<strg.length; char_pos++) {
    if (strg.charAt(char_pos) == "|" || strg.charAt(char_pos) == "&" || strg.charAt(char_pos) == "=" || strg.charAt(char_pos) == '"' || strg.charAt(char_pos) == "'") {
       found = 1;
       break;
    }
 }
 return (found == 1) ? false : true;
}
function set_value(entry, old_value) {
 var new_value = window.prompt(entry+" の値を変更します",old_value)
 var found = 0;
 if (new_value != "" && new_value != null) {
    for (char_pos=0; char_pos<new_value.length; char_pos++) {
       if (!(new_value.charAt(char_pos) >= '0' && new_value.charAt(char_pos) <= '9')) {
          found = 1;
       }
    }
    if (found == 1) {
       alert ("数字を入力してください！");
    }
    else if (new_value == old_value) {
       alert ("変更されてません！");
    }
    else { 
       window.location.href = "<? echo $jump;?>&act=set_value&fid="+entry+"&new="+new_value
    }
 }
}
function ChangeEntry(entry, old_name, feature) {
 var new_name = window.prompt(entry+"　の新しい "+feature+" を入力してください",old_name)
 var found = 1;
 if (new_name != "" && new_name != null) {
    if (check_char(new_name)) {
         found = 0;
    }
    if (found == 1) {
       alert ("Error! "+entry+" に許可されてない文字が含まれています!");
    }
    else if (new_name == old_name) {
       alert ("変更されてません！");
    }
    else if (feature == "説明") {
       window.location.href = "<? echo $jump;?>&act=ren_entry&fid="+entry+"&new="+new_name
    }
    else if (feature == "FileID") {
       window.location.href = "<? echo $jump;?>&act=ren_id&fid="+entry+"&new="+new_name
    }
    else if (feature == "URL") {
       window.location.href = "<? echo $jump;?>&act=set_loc&fid="+entry+"&new="+new_name
    }
 }
}
function del_entry(entry) {
 if (window.confirm("本当に削除してもよろしいですか？("+entry+")")) {
    window.location.href = "<? echo $jump;?>&act=del_entry&fid="+entry
 }
}
function check_form() {
 var found = 1;
 des_field=document.form.description.value;
 id_field=document.form.new_id.value;
 if (des_field == "" || id_field == "") {
    alert ("IDとタイトルを埋めてください！");
    return false;
 }
 if (check_char(des_field) && check_char(id_field)) {
    found = 0;
 }
 if (found == 1) {
    alert ("Error! 許可されていない文字です");
    return false;
 }
 if (document.form.new_loc.value.indexOf(".")==-1) {
    alert("ファイルの場所が正しくありません！");
    document.form.new_loc.focus();
    return false;
 }
}
// -->
</script>
</head>
<body bgcolor="#FFFFFF">
 <table width="560" border="0" align="center">
  <tr>
    <td height="40"><tt>Administration Mode</tt></td>
    <td height="40" align="right" valign="bottom"><img src="point.gif" width="9" height="9"><a href="<? echo $PHP_SELF;?>?">Back to Top Linkers</a></td>
  </tr>
 </table>
 <table bgcolor="#F7F7F7" border=1 cellspacing=0 cellpadding=3 width="560" align="center">
    <tr bgcolor="#E1A8C1"> 
      <td><b>説明</b></td>
      <td><b>ID</b></td>
      <td><b>DL数</b></td>
      <td><b>場所</b></td>
      <td><b>削除</b></td>
    </tr>
<?php
$lines = file(LOGFILE);
$totalfiles = count($lines);
for($i=0; $i<count($lines); $i++){
  list($file_id,$count,$location,$stamp,$descript) = explode("|",$lines[$i]);
  if($location=="") $location = "-";
  if($refurl=="") $refurl="-";
  echo "<tr>\n<td><a href=\"javascript:ChangeEntry('$file_id','$descript','説明')\">$descript</a></td>\n      <td><a href=\"javascript:ChangeEntry('$file_id','$file_id','FileID')\">$file_id</a></td>\n";
  echo "<td><a href=\"javascript:set_value('$file_id','$count')\">$count</a></td>\n      <td><a href=\"javascript:ChangeEntry('$file_id','$location','URL')\">$location</a></td>\n"; 
  echo "<td><a href=\"javascript:del_entry('$file_id')\">削除</a></td>\n    </tr>\n";
  $total+=$count;
}
echo '
    <tr> 
      <td><b><font color="#336600">&gt;&gt;Total Files:</font></b> <font color="#FF0000">'.$totalfiles.'</font></td>
      <td colspan="4"><b><font color="#336600">&gt;&gt;Total Linkers:</font></b> <font color="#FF0000">'.$total.'</font></td>
    </tr>
  </table>
  <form method="post" action="'.$_SERVER['PHP_SELF'].'" name="form">
    <table width="560" border="0" align="center">
      <tr>
';?>
        <td>リンク先のURL： </td>
        <td><input type="text" name="new_loc" size="42"><small>http://から</small>
          <input type="hidden" name="job" value="admin">
          <input type="hidden" name="admin" value="<? echo md5(ADMINPASS);?>">
      </tr>
      <tr> 
        <td>ID： </td>
        <td><input type="text" name="new_id" size="42"></td>
      </tr>
      <tr> 
        <td>タイトル： </td>
        <td><input type="text" name="description" size="42"></td>
      </tr>
      <tr> 
        <td></td>
        <td>
          <input type="submit" name="submit" value="Add Entry" onclick="return check_form()" class="input"></td>
      </tr>
    </table>
  </form>
<center>リンクの仕方　link.php?id=[ID]</center>
</body>
</html>
<?
exit;
}
/* 追加・変更・削除 */
function manage($manage,$id,$new){
  $lines = file(LOGFILE);//ログ読み込み
  $find = FALSE;//フラグ

  for($i=0; $i<count($lines); $i++){
    //データ分解
    list($file_id,$count,$location,$stamp,$descript) = explode("|",$lines[$i]);
    if($id == $file_id){//同じIDだったら
      switch($manage){	//モード分岐
      case 'del_entry':	//削除
        $lines[$i] = "";
        $find = TRUE;
        break;
      case 'ren_entry':	//説明変更
        $new = chop($new);
        if(get_magic_quotes_gpc()) $new = stripslashes($new);
        $lines[$i] = "$file_id|$count|$location|$stamp|$new|\n";
        $find = TRUE;
        break;
      case 'ren_id':	//ID変更
        $new = preg_replace("/\s*/","",$new);
        $lines[$i] = "$new|$count|$location|$stamp|$descript|\n";
        $find = TRUE;
        break;
      case 'set_value':	//カウント変更
        $lines[$i] = "$file_id|$new|$location|$stamp|$descript|\n";
        $find = TRUE;
        break;
      case 'set_loc':	//場所変更
        $new = preg_replace("/\s*/","",$new);
        $lines[$i] = "$file_id|$count|$new|$stamp|$descript|\n";
        $find = TRUE;
        break;
      }
    }
  }
  if($find){//ログ更新
    $fp = fopen(LOGFILE, "w");
    flock($fp, LOCK_EX);
    fputs($fp, implode("", $lines));
    fclose($fp);
  }
}
/* 新規追加 */
function add_data($url,$id,$desc){
  //整形
  $id = str_replace("|","",$id);
  $id = str_replace("'","",$id);
  $id = chop(str_replace('"','',$id));
  $url = chop($url);
  if(strstr($url,".") && !empty($id)) {
    $lines = file(LOGFILE);
    $find = FALSE;
    //同じIDが登録されていないか
    for($i=1; $i<count($lines); $i++){
      list($file_id,) = explode("|",$lines[$i]);
      if($id == $file_id){
        $find = TRUE;
        break;
      }
    }
    if(!$find){//ログ書き込み
      $desc = str_replace("ポ","ﾎﾟ",$desc);
      $desc = str_replace("|","",$desc);
      $desc = str_replace("'","",$desc);
      $desc = chop(str_replace('"','',$desc));
      if(get_magic_quotes_gpc()) $desc = stripslashes($desc);
      $stamp = time();
      $newline = "$id|0|$url|$stamp|$desc|\n";//フォーマット
      $fp = fopen(LOGFILE, "a");
      fputs($fp, $newline);
      fclose($fp);
    }
  }else{
    die("URLかIDが不正です");
  }
}
/* 管理モードログイン */
function validation($admin){

  if($admin && ($admin != md5(ADMINPASS)))
    $err = "<font color=red>パスワードが違います！</font>";
  if(empty($admin) || $err){
    ?>
      <html>
        <head><title>Link Counter</title>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
<style type="text/css">
<!--
td {  font-family: Arial, Helvetica, sans-serif; font-size: 8pt}
tt {  font-family: Verdana, Arial, Helvetica, sans-serif; font-size: 13pt; font-weight: bold}
-->
</style>
</head>
<body bgcolor="FFEEFF">
<form method="post" action="<? echo $_SERVER['PHP_SELF'];?>">
 <table width="560" border="0" align="center">
  <tr>
    <td height="40"><tt>Administration Mode</tt></td>
  </tr>
 </table>
 <table width="560" border="0" cellspacing="0" cellpadding="3" align="center">
   <tr bgcolor="E1A8C1">
     <td><b>パスワードを入力してください</b></td>
   </tr>
   <tr bgcolor="F7F7F7"> 
     <td> 
       <input type="password" name="admin_submit">
       <input type="submit" value="Submit">
       <input type="hidden" name="job" value="admin">
     </td>
   </tr>
   <tr bgcolor="F7F7F7"> 
     <td><? echo $err;?></td>
   </tr>
   <tr> 
     <td><br>
       レッツPHP!(http://php.s3.to)
       | <a href="<? echo $PHP_SELF;?>?">Back to Top Linkers</a></td>
   </tr>
 </table>
</form>
</body>
</html>
<?php
exit;
  }
  return true;
}
/*************main******************/
//ダウンロード
if(!empty($_GET['id'])){
  if(REFCHECK){
    for($i=0; $i<count($okurl); $i++){
      if(!strstr(getenv("HTTP_REFERER"),$okurl[$i]))
        die("指定URL以外からはLinkできません");
    }
  }
  count_dl($_GET['id']);
  exit;
}
switch($_REQUEST['job']){
case 'detail':
  if($_GET['detail']) show_detail($_GET['detail']);
  break;
case 'admin':
  $admin = $_REQUEST['admin'];
  if($_POST['admin_submit']) $admin = md5($_POST['admin_submit']);
  validation($admin);
  if($_GET['act']) manage($_GET['act'],$_GET['fid'],$_GET['new']);
  if($_POST['submit']=="Add Entry")
    add_data($_POST['new_loc'],$_POST['new_id'],$_POST['description']);
  show_track();
  break;
default:
  if(time()-filemtime(LOGFILE) > 3600) @copy(LOGFILE,LOGFILE.".bak");
  show_table($_POST['top'],$_POST['option']);
}
?>
