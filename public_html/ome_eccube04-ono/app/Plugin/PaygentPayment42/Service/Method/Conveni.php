...
namespace Plugin\PaygentPayment42\Service\Method;
...
use Plugin\PaygentPayment42\Service\PaygentRequestService;
...
class Conveni extends AbstractController implements PaymentMethodInterface
{
    ...
    public function sendConveniOrderMail($order, $arrRet)
    {
        ...
        $message = (new Email())
            ->subject('['.$this->baseInfo->getShopName().'] ' . $this->eccubeConfig['paygent_payment']['temp_order_mail_title'])
            ->from(new Address($this->baseInfo->getEmail04(), $this->baseInfo->getShopName()))
            ->to($order->getEmail())
            ->bcc($this->baseInfo->getEmail01())
            ->replyTo($this->baseInfo->getEmail03())
            ->returnPath($this->baseInfo->getEmail04())
            ->text($this->renderView("@PaygentPayment42/default/Shopping/conveni_mail.twig",$params));
