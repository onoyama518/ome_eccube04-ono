...
class FrontConveniService extends PaygentBaseService {
    ...
    public function makeUsableCvsCompanyName($cvsCompanyId)
    {
        ...
        // コンビニ名称の設定、A/B/D使用コンビニ企業コードとF使用コンビニ企業コードとは混在させない
        foreach ($arrCvsCompanyId as $cvsCompanyId) {
            if ($config->getConveniConnectType() === $this->eccubeConfig['paygent_payment']['cvs_connect_type']['f']) {
                if (isset($this->eccubeConfig['paygent_payment']['cvs_type_f_company_id_to_name'][$cvsCompanyId])) {
                    $usableCvsCompanyName .= $this->eccubeConfig['paygent_payment']['cvs_type_f_company_id_to_name'][$cvsCompanyId];
                    $usableCvsCompanyName .= '、';
                }
            } else {
                $usableCvsCompanyKey = array_search($cvsCompanyId, $this->eccubeConfig['paygent_payment']['cvs_company_id']);
                if (isset($this->eccubeConfig['paygent_payment']['cvs_company_name'][$usableCvsCompanyKey])) {
            ...
            if ($cvsCompanyId === end($arrCvsCompanyId)){
                $usableCvsCompanyName = rtrim($usableCvsCompanyName, '、');
            }
        }
