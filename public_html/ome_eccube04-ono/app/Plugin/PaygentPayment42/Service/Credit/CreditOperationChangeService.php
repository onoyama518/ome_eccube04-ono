...
use Plugin\PaygentPayment42\jp\co\ks\merchanttool\connectmodule\system\PaygentB2BModule;
...
class CreditOperationChangeService extends CreditOperationService {
    ...
    /**
     * @var CreditChangeCommitValidation
     */
    protected $creditChangeCommitValidation;
     ...
     * @param CreditChangeCommitValidation $creditChangeCommitValidation
     */
    public function __construct(
        ...
        CreditChangeCommitValidation $creditChangeCommitValidation
    ) {
        ...
        $this->creditChangeCommitValidation = $creditChangeCommitValidation;
    }
    ...
    public function process($paygentType, $orderId, $paymentId = '', $beforeStatus = '')
    {
        ...
        // 決済情報照会電文を行い消込済みでない場合売上処理を中断する
        if ($paygentType === "change_commit") {
            if (!$this->creditChangeCommitValidation->canChengeCommit($arrSendParam)) {
                $arrReturn['return'] = false;
                $arrReturn['kind'] = $arrPaygentKind['paygent_card_commit_revice'];
                $arrReturn['response'] = $this->creditChangeCommitValidation->getErrorMessage();
                return $arrReturn;
            }
        }
