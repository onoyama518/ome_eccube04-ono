...
use Plugin\PaygentPayment42\Repository\Credit3dSecure2OrderRepository;
...
class FrontCredit3dSecure2Service extends FrontCreditService
{
    ...
    private function setParamsForCustomer($params, $inputOrder, $customer, $sysDateTime)
    {
        ...
        // クレジットカード支払いを取得
        $payment = $this->paymentRepository->findOneBy(['method_class' => 'Plugin\\PaygentPayment42\\Service\\Method\\Module\\Credit']);
    ...
    /**
     * Attempt区分チェック処理
     * @param string $attemptKbn
     *
     * @return boolean チェック結果(true:オーソリを実行する/false:オーソリを実行しない)
     */
    public function checkAttemptKbn($attemptKbn)
    {
        $config = $this->cacheConfig->getConfig();
        $const = $this->eccubeConfig['paygent_payment'];
        $attemptAuthKbn = $config->getAttemptAuthKbn();

        // Attemptオーソリ制御区分が0(全て実施しない)の場合
        if ($attemptAuthKbn === $const['attempt_auth_kbn']['auth_none']){
            // Attempt区分がnullの場合
            if( $attemptKbn === "" ) {
                return true;
            }
        // Attemptオーソリ制御区分が1(正常時のみ実施)の場合
        } elseif ($attemptAuthKbn === $const['attempt_auth_kbn']['auth_success']){
            // Attempt区分がnullか0(正常)の場合
            if( $attemptKbn === "" || $attemptKbn === $const['attempt_kbn']['success'] ) {
                return true;
            }
        // Attemptオーソリ制御区分が2(全て実施)の場合
        } elseif ($attemptAuthKbn === $const['attempt_auth_kbn']['auth_all']){
            // Attempt区分がnullか0(正常)か1(注意)の場合
            if( $attemptKbn === "" || $attemptKbn === $const['attempt_kbn']['success'] || $attemptKbn === $const['attempt_kbn']['error'] ) {
                return true;
            }
        }
        return false;
