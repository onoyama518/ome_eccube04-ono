...
namespace Plugin\PaygentPayment42\Service\Paidy;
...
class PaidyService extends PaygentBaseService
{
    ...
    public function sendPaidyMismatchMailToAdmin($arrParam, $paymentAmount)
    {
        ...
        $body = $this->twig->render("@PaygentPayment42/admin/paidy_mismatch_mail.twig", [
        ...
        $message = (new Email())
            ->subject('[' . $this->baseInfo->getShopName() . '] ' . $this->eccubeConfig['paygent_payment']['paidy_mismatch_mail_title'])
            ->from(new Address($this->eccubeConfig['paygent_payment']['paygent_support_mail_address']))
    ...
    function isAlertMail($settlementDivision, $paymentStatus, $order)
    {
        ...
        if (
            $paymentStatus == $this->eccubeConfig['paygent_payment']['status_authority_ok'] &&
