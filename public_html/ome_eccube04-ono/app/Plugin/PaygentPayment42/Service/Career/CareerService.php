...
namespace Plugin\PaygentPayment42\Service\Career;
...
class CareerService extends PaygentBaseService
{
    ...
    public function updatePaygentOrder($arrRet)
    {
        ...
        // 消込済・消込完了かつ支払日時が含まれる
        if (
            in_array($arrRet['payment_status'], [$this->eccubeConfig['paygent_payment']['status_pre_cleared'], $this->eccubeConfig['paygent_payment']['status_complete_cleared']], true)
            && $arrRet['payment_date'] != ""
        ) {
            // 入金日時 = 応答情報.支払日時
