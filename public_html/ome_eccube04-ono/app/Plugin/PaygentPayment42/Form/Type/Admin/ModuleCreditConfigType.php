...
namespace Plugin\PaygentPayment42\Form\Type\Admin;

use Eccube\Common\EccubeConfig;
use Plugin\PaygentPayment42\Entity\Config;
use Plugin\PaygentPayment42\Form\Type\Admin\ConfigType;
...
class ModuleCreditConfigType extends ConfigType
{
    ...
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        ...
        if ($isCredit && $isModule) {
            ...
            if ($isCredit3d != $this->eccubeConfig['paygent_payment']['credit_3d']['3dSecure_off']) {
                ...
                if ($isCredit3d == $this->eccubeConfig['paygent_payment']['credit_3d']['3dSecure_2']) {
                    $arrFormConstraints += [
                        'attempt_auth_kbn' => [
                            new NotBlank(['message' => '※ Attemptオーソリ制御区分が入力されていません。']),
                        ],
                    ];
        ...
        $builder->add($builder->create('payment_division', ChoiceType::class, [
            ...
            'expanded' => true,
            'multiple' => true,
            'constraints' => $arrFormConstraints['payment_division'] ?? [],
        ])->addModelTransformer($this->getTransformer()));

        $builder->add('security_code', ChoiceType::class, [
            'choices' => ['要' => 1,'不要' => 0],
            'expanded' => true,
            'constraints' => $arrFormConstraints['security_code'] ?? [],
        ]);
        $builder->add('credit_3d', ChoiceType::class, [
            ...
            'expanded' => true,
            'constraints' => $arrFormConstraints['credit_3d'] ?? [],
        ]);
        $builder->add('attempt_auth_kbn', ChoiceType::class, [
            'choices' => [
                '全て実施しない' => $this->eccubeConfig['paygent_payment']['attempt_auth_kbn']['auth_none'],
                '正常時のみ実施' => $this->eccubeConfig['paygent_payment']['attempt_auth_kbn']['auth_success'],
                '全て実施'      => $this->eccubeConfig['paygent_payment']['attempt_auth_kbn']['auth_all'],
            ],
            'expanded' => true,
            'constraints' => $arrFormConstraints['attempt_auth_kbn'] ?? [],
        ]);
        $builder->add('module_stock_card', ChoiceType::class, [
            'choices' => ['要' => 1,'不要' => 0],
            'expanded' => true,
            'constraints' => $arrFormConstraints['module_stock_card'] ?? [],
        ]);
        $builder->add('token_env', ChoiceType::class, [
            'choices' => $arrCreditTokenEnvDivisions,
            'expanded' => true,
            'constraints' => $arrFormConstraints['token_env'] ?? [],
        ]);
        $builder->add('token_key', TextType::class, [
            'constraints' => $arrFormConstraints['token_key'] ?? [],
        ]);
        $builder->add('credit_3d_hash_key', TextType::class, [
            'constraints' => $arrFormConstraints['credit_3d_hash_key'] ?? [],
        ]);
        $builder->add('credit_3d_merchant_name', TextType::class, [
            'constraints' => $arrFormConstraints['credit_3d_merchant_name'] ?? [],
        ]);
        $builder->add('card_valid_check', ChoiceType::class, [
            'choices' => ['要' => 1,'不要' => 0],
            'expanded' => true,
            'constraints' => $arrFormConstraints['card_valid_check'] ?? [],
        ]);
    ...
    public function setDataForDisabledCredit(FormEvent $event) {
        ...
        if ($requestData['credit_3d'] == $this->eccubeConfig['paygent_payment']['credit_3d']['3dSecure_off']) {
            $requestData['attempt_auth_kbn'] = $form['attempt_auth_kbn']->getData();
