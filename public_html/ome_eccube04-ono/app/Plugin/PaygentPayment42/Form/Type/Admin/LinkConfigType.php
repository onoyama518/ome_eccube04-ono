...
class LinkConfigType extends ConfigType
{
    ...
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        ...
        $builder->add('link_url', UrlType::class, [
            'constraints' => $arrFormConstraints['link_url'] ?? [],
        ]);
        $builder->add('hash_key', TextType::class, [
            'required' => false,
            'constraints' => $arrFormConstraints['hash_key'] ?? [],
        ]);
        $builder->add('link_payment_term', TextType::class, [
            ...
            'constraints' => $arrFormConstraints['link_payment_term'] ?? [],
        ]);
        $builder->add(
            $builder
                ->create('merchant_name', TextType::class, [
                    'required' => false,
                    'constraints' => $arrFormConstraints['merchant_name'] ?? [],
                ])
                ->addEventSubscriber(new \Eccube\Form\EventListener\ConvertKanaListener('AK'))
        );
        $builder->add('link_copy_right', TextType::class, [
            'required' => false,
            'constraints' => $arrFormConstraints['link_copy_right'] ?? [],
        ]);
        $builder->add(
            $builder
                ->create('link_free_memo', TextType::class, [
                    'required' => false,
                    'constraints' => $arrFormConstraints['link_free_memo'] ?? [],
                ])
                ->addEventSubscriber(new \Eccube\Form\EventListener\ConvertKanaListener('AK'))
        );
        $builder->add('return_url', UrlType::class, [
            'required' => false,
            'constraints' => $arrFormConstraints['return_url'] ?? [],
        ]);
