 ...
 */
 ...
 namespace Plugin\PaygentPayment42\Controller;
...
use Plugin\PaygentPayment42\Form\Type\CreditPaymentType;
use Plugin\PaygentPayment42\Service\Credit\FrontCreditService;
use Sensio\Bundle\FrameworkExtraBundle\Configuration\Route;
use Sensio\Bundle\FrameworkExtraBundle\Configuration\Template;
use Symfony\Component\HttpFoundation\Request;
use Symfony\Component\HttpFoundation\Response;
use Symfony\Component\Form\FormError;
use Plugin\PaygentPayment42\Service\CacheConfig;
use Plugin\PaygentPayment42\Service\Credit\FrontCredit3dSecure2Service;
use Plugin\PaygentPayment42\Service\Credit\FrontCreditCardStockService;
...
class PaymentCreditController extends AbstractController
{
     ...
     * @Template("@PaygentPayment42/default/payment_credit.twig")
     */
    public function index(Request $request)
    {
        ...
        // 登録カード削除状態でなくカード未選択の場合エラー追加
        if ($form->isSubmitted()){
            if($inputParams['delete_card'] ==='0' && $inputParams['stock'] && is_null($request->request->get('customer_card_id'))) {
                $form['stock']->addError(new FormError('登録カードが選択されていません。'));
            }
        }
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($inputParams['delete_card']) {
                ...
                if ($cardErrorCount < $this->eccubeConfig['paygent_payment']['credit_authority_retry_limit']) {
                    ...
                    if ($config->getCredit3d() === (int)$this->eccubeConfig['paygent_payment']['credit_3d']['3dSecure_2']) {
                        ...
                        // 3Dセキュア2.0認証申込みの結果が異常かつAttemptオーソリ制御区分が全て実施の場合は1.0のオーソリ電文を呼び出す
                        if ($ret['resultStatus'] === $this->eccubeConfig['paygent_payment']['result_error'] && $config->getAttemptAuthKbn() === $this->eccubeConfig['paygent_payment']['attempt_auth_kbn']['auth_all']) {
        ...
        $arrReturn = [
            ...
            'isCredit3d2' => $config->getCredit3d() === (int)$this->eccubeConfig['paygent_payment']['credit_3d']['3dSecure_2']
        ];

        return $this->render('@PaygentPayment42/default/payment_credit.twig', $arrReturn, $response);
    ...
    public function credit3d2Secure(Request $request)
    {
        ...
        // 3Dセキュア2.0Attempt区分 null, 0=正常. 1=注意
        $attemptKbn = $request->get('attempt_kbn');
        ...
        // 3Dセキュア2.0認証結果応答電文の処理結果が正常かつAttempt区分がAttemptオーソリ制御区分設定でオーソリ可能な対象範囲の場合
        if ($resultStatus === $this->eccubeConfig['paygent_payment']['result_success'] && $this->frontCredit3dSecure2Service->checkAttemptKbn($attemptKbn)) {
        ...
        // 2.3Dセキュア2.0認証結果応答電文の処理結果が異常またはAttempt区分がAttemptオーソリ制御区分設定でオーソリ可能な対象範囲外の場合
        } else {
            // 異常終了時、レスポンスコードが取得できる
            $responseCode = $request->get('response_code');
            ...
            if ($resultStatus === $this->eccubeConfig['paygent_payment']['result_success']) {
                $this->errorLogOutput($credit3D2Res, "3Dセキュア2.0認証結果応答電文エラー：Attempt区分が不正です。");
            } else {
        ...
        } elseif ($resultStatus === $this->eccubeConfig['paygent_payment']['result_success']) {
            $error = "決済に失敗しました。<br>Attempt区分が不正です。";
        } else {
            $error = "決済に失敗しました。";
        }
