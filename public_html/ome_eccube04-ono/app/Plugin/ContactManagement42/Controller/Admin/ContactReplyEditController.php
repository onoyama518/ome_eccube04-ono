...
use Plugin\ContactManagement42\Form\Type\Admin\ContactReplyType;
use Plugin\ContactManagement42\Repository\ContactReplyRepository;
use Plugin\ContactManagement42\Repository\ContactRepository;
use Plugin\ContactManagement42\Service\MailService;
...
class ContactReplyEditController extends AbstractController
{
     ...
     * @ParamConverter("Contact", class="Plugin\ContactManagement42\Entity\Contact", options={"id" = "contact_id"})
     * @Template("@ContactManagement42/admin/reply_edit.twig")
    ...
    public function index(Request $request, Contact $Contact, $id = null)
    {
        ...
        $this->eventDispatcher->dispatch($event, 'admin.contact.reply_edit.index.initialize');
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->eventDispatcher->dispatch($event, 'admin.contact.reply_edit.index.complete');
            ...
            if ($request->get('mode') === 'send') {
                ...
                if ($message) {
                    ...
                    $this->addError('メールを送信できませんでした。', 'admin');
                }
